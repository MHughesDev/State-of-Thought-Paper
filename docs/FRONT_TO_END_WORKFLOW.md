...
4. Mark unknowns explicitly as `UNKNOWN`, `N/A`, or with the unresolved markers required by the spec.
5. Add only authorized source materials to the repository.
6. Use `templates/quick_spec_short_form.md` only as a fast-fill helper, then merge the answers back into `PROJECT_SPEC.md`.
