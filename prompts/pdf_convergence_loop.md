...
3. `paper/main.pdf` opened in the editor: no overlapping text, cropped figures, spilled tables, or broken equations in margins. For TikZ overlap checks without relying on the PDF viewer alone, use `docs/FIGURE_VISUAL_AUDIT.md` and `python scripts/figure_visual_audit.py` (requires `pip install pymupdf`).
...
5. Edit LaTeX under `paper/` (and `paper/bib/` if needed). Prefer minimal fixes: line breaks, `\sloppy`/`microtype`, resizebox only when justified, fix cite keys and refs.
6. Update `docs/PDF_REVIEW.md` **Manual Findings** with what you fixed or what remains blocked.
7. If placeholders or citations are involved, run `python scripts/check_placeholders.py`, `python scripts/check_bib_placeholders.py`.
8. Go to step 1 until steps in **Exit criteria** pass.
