...
11. Before calling a compile-ready pass complete, build `paper/main.pdf`, run `python scripts/render_audit.py --write-full-extract` when possible (for `docs/PDF_TEXT_EXTRACT.md`), run `python scripts/pdf_quality_gate.py --skip-build` to check `paper/main.log`, review the PDF visually, and follow `prompts/pdf_convergence_loop.md` until render-quality exit criteria are met or blockers are explicit. For **figure overlap/collision** review without relying on text extract, use `pip install pymupdf`, then `python scripts/figure_visual_audit.py` and `prompts/figure_visual_audit_loop.md` (see `docs/FIGURE_VISUAL_AUDIT.md`).
12. Never claim the project is complete while critical placeholders or unresolved blockers remain.
