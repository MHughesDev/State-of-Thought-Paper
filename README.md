...
> **Manuscript in this workspace:** *State of Thought: A Theoretical Architecture for Persistent Multi-Agent Cognition* — LaTeX under `paper/main.tex`; build via `python scripts/build_paper.py`; handoff in `docs/RELEASE_NOTES.md`; checklist in `docs/RELEASE_CHECKLIST.md`.
...
8. For a strict PDF pass: `python scripts/pdf_quality_gate.py` (build + full-text extract + log gate), or after verify: `python scripts/pdf_quality_gate.py --skip-build`. Use `make verify-gate` to chain verify and the gate. Use `STRICT=1 make quality-gate` to fail on overfull boxes. Repeat fixes using `prompts/pdf_convergence_loop.md` until clean.
